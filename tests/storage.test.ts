...
describe("PostgresBoardStore", () => {
  it("requires unique 1 to 3 capital letter project IDs", async () => {
    await expect(store.createProject({ title: "No key" })).rejects.toThrow(
      "Project ID is required",
    );
    await expect(
      store.createProject({ title: "Invalid", key: "app" }),
    ).rejects.toThrow("Project ID must be 1 to 3 capital letters");

    const project = await store.createProject({ title: "App", key: "APP" });
    expect(project.key).toBe("APP");

    await expect(
      store.createProject({ title: "Duplicate", key: "APP" }),
    ).rejects.toThrow("Project ID APP is already used");
    ...
    expect(
      (await store.updateProject(otherProject.id, { key: "WEB" })).key,
    ).toBe("WEB");
    await expect(
      store.updateProject(otherProject.id, { key: "APP" }),
    ).rejects.toThrow("Project ID APP is already used");
  });
  ...
  it("assigns incrementing task numbers per project", async () => {
    ...
    const firstAppTask = await store.createIdea(admin, {
      projectId: app.id,
      title: "First app task",
    });
    const secondAppTask = await store.createIdea(admin, {
      projectId: app.id,
      title: "Second app task",
    });
    const firstApiTask = await store.createIdea(admin, {
      projectId: api.id,
  ...
  it("stores tasks inside a project", async () => {
    ...
    expect(
      (await store.listIdeas(admin, project.id)).map((item) => item.id),
    ).toEqual([idea.id]);
  });
  ...
  it("refines an idea when details and acceptance criteria are updated", async () => {
    ...
    expect(refined.acceptanceCriteria).toEqual([
      "Ready ideas can be selected",
      "A board card is created",
    ]);
  });
  ...
  it("uses ready as the flag for board availability", async () => {
    ...
    expect((await store.getBoardColumns(admin, project.id)).ready).toHaveLength(
      1,
    );
    expect(
      (await store.getBoardColumns(admin, project.id)).ready[0].ideaId,
    ).toBe(idea.id);

    const unavailable = await store.setIdeaBoardAvailability(
      admin,
      idea.id,
      false,
    );
  ...
  it("moves board cards between any columns without workflow constraints", async () => {
    ...
    expect((await store.moveBoardCard(admin, card.id, "done")).column).toBe(
      "done",
    );
    expect((await store.moveBoardCard(admin, card.id, "planned")).column).toBe(
      "planned",
    );
  });
  ...
  it("stores the implementation branch name on board cards", async () => {
    ...
    expect(
      (await store.getBoardColumns(admin, project.id)).ready[0].branchName,
    ).toBe("vp-task-lifecycle-state");
  });
  ...
  it("syncs edited task details and repository metadata to an existing board card", async () => {
    ...
    const [updatedCard] = (await store.getBoardColumns(admin, project.id))
      .ready;
  ...
  it("stores execution plan documents linked to ideas and board cards", async () => {
    ...
    const card = await store.createBoardCardFromIdea(
      admin,
      (await store.markIdeaReady(admin, idea.id)).id,
      {
        githubMode: "local",
      },
    );
  ...
  it("exports scoped state for admins and project tokens", async () => {
    ...
    const scopedState = await store.getState({
      kind: "token",
      tokenId: "token-1",
      projectIds: [project.id],
    });

    expect(fullState.projects.map((item) => item.id).sort()).toEqual(
      [otherProject.id, project.id].sort(),
    );
    expect(scopedState.projects.map((item) => item.id)).toEqual([project.id]);
    expect(scopedState.ideas.map((item) => item.title)).toEqual([
      "Scoped task",
    ]);
  });
  ...
  it("sets readiness on a board card without bumping updated_at", async () => {
    ...
    expect(scored.readinessReason).toBe(
      "Clear scope, acceptance criteria present",
    );
  ...
  it("rejects invalid readiness scores and empty reasons", async () => {
    ...
    await expect(
      store.setCardReadiness(admin, card.id, { score: 0, reason: "r" }),
    ).rejects.toThrow("Readiness score must be an integer from 1 to 10");
    await expect(
      store.setCardReadiness(admin, card.id, { score: 11, reason: "r" }),
    ).rejects.toThrow("Readiness score must be an integer from 1 to 10");
    await expect(
      store.setCardReadiness(admin, card.id, { score: 6.5, reason: "r" }),
    ).rejects.toThrow("Readiness score must be an integer from 1 to 10");
    await expect(
      store.setCardReadiness(admin, card.id, { score: 5, reason: "  " }),
    ).rejects.toThrow("Readiness reason is required");
  });
  ...
  it("marks readiness stale after a content edit but not after re-evaluation", async () => {
    ...
    const edited = await store.updateBoardCard(admin, card.id, {
      details: "Now with repo info",
    });
